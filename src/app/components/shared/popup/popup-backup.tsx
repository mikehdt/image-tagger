import React, { useEffect, useRef   import React, { useEffect, useRef } from 'react';
...
};port { usePopup } from './popup-context';
import { PopupProps } from './types';
import {
  calculatePopupPosition,
  DEFAULT_OFFSET,
...
export const Popup: React.FC<PopupProps> = ({
  ...
  useEffect(() => {
    setPopupConfig(id, { position, triggerRef, offset, className });
  }, [id, position, triggerRef, offset, className, setPopupConfig]); // Calculate position when popup opens
  useEffect(() => {
    if (state.shouldRender && popupRef.current && config?.triggerRef?.current) {
      const result = calculatePopupPosition(
        config.triggerRef.current,
      ...
      // Apply calculated styles
      Object.assign(popupRef.current.style, result.styles);

      // Set transform origin for smooth animation
      popupRef.current.style.transformOrigin = getTransformOrigin(
        result.position,
      );
    }
  }, [state.shouldRender, config, position, offset]);
